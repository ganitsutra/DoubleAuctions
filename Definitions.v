...
Definition ord_eqbP : forall n m : order, {n = m} + { n <> m}.
...
Lemma nodup_timesof_uniq_order (B:list order)(b1 b2 :order): 
NoDup (timesof B) -> In b1 B ->In b2 B-> otime b1 = otime b2 -> b1 = b2.
Proof. revert b1 b2. induction B as [|b B0]. simpl. intros. inversion H1.
       intros. destruct H0;destruct H1. { subst.  auto. } { subst. apply 
       timesof_elim in H1.  simpl in H. rewrite H2 in H. assert(~In (otime b2) 
       (timesof B0)). eauto. destruct (H0 H1). } { subst. apply timesof_elim in H0.
       simpl in H. rewrite H2 in H0. assert(~In (otime b2) (timesof B0)). eauto.
       destruct (H1 H0). } { apply IHB0.  all: auto.  eauto. } Qed.
...
Definition Qty_orders (B: list order):nat:=
...
Proof. intros. apply included_intro. intros. assert(Hp1:= H). 
       ...
       (delete_order B2 i) <= count a B2). assert(i = 
...
(ida x ==  ida y)&&  (tprice x == tprice y) &&  (tquantity x == tquantity y).

(*------------------ Attaching transaction to eqType ------------------------------ *)

Lemma t_eqb_ref (x: transaction): t_eqb x x = true.
Proof. unfold t_eqb. apply /andP. split. apply /andP. split. apply /andP. split. all: apply /eqP; auto. Qed.

Lemma t_eqb_elim (x y: transaction):  t_eqb x y -> x = y.
Proof. { unfold t_eqb. destruct x. destruct y. simpl. intros. move /andP in H.
         destruct H. move /andP in H. destruct H. move /andP in H. destruct H. move /eqP in H. move /eqP in H1.
         move /eqP in H0. move /eqP in H2. subst. f_equal. rewrite (BoolDecidableEqDepSet.UIP _ _ 
         tquantity_cond0 tquantity_cond1).  auto. } Qed.

Lemma t_eqb_intro (x y: transaction): x=y -> t_eqb x y = true.
Proof. { unfold t_eqb. intros H. apply /andP. split. apply /andP. split. apply /andP. 
         split. apply /eqP. subst x. exact. apply /eqP. subst x. exact. apply /eqP. 
         subst x. apply /eqP.  exact.  apply /eqP. subst x. exact. } Qed.  
...
Definition sum L:= fold_right Nat.add 0 L.

Definition Qty_bid2 T i := sum (map tquantity (filter (fun t => (idb t) == i) T)).

Definition Vol1 T := sum (map tquantity T).
Definition fun_ids_bid T := uniq  (map idb T).
Lemma Qty_bid_equal T i:
Qty_bid2 T i = Qty_bid T i.
Proof. unfold Qty_bid2. unfold sum. induction T. simpl. simpl. auto. 
simpl. destruct (idb a =? i) eqn:Hi. simpl. rewrite <- IHT. auto.
rewrite <- IHT.  auto. Qed.
*)
...
Lemma Qty_le_Qty_ask (T: list transaction) (i j:nat): 
Qty T i j <= Qty_ask T j. 
Proof. induction T. simpl. lia. simpl. destruct ((idb a =? i)) eqn:Hi;destruct (ida a =? j) eqn:Hj.
{ simpl;lia. } {simpl;lia. } {simpl;lia. } {simpl;lia. } Qed.

Lemma Qty_le_Qty_bid (T: list transaction) (i j:nat): 
Qty T i j <= Qty_bid T i. 
Proof. induction T. simpl. lia. simpl. destruct ((idb a =? i)) eqn:Hi;destruct (ida a =? j) eqn:Hj.
{ simpl;lia. } {simpl;lia. } {simpl;lia. } {simpl;lia. } Qed.


Lemma Qty_lt_Qty_bid_m_exists (T: list transaction) (i j:nat):
Qty T i j < Qty_bid T i ->
exists m : transaction,
  In m T /\ idb m = i /\ ida m <> j.
Proof. induction T. simpl. intros. lia.
intros. simpl in H. destruct ((idb a =? i)) eqn:Hi;destruct (ida a =? j) eqn:Hj.
{ simpl in H. assert(Qty T i j < Qty_bid T i). lia. apply IHT in H0. destruct H0. exists x.
  split. simpl. right. apply H0. apply H0. }
{ exists a. split. simpl. auto. move /eqP in Hi. move /eqP in Hj. auto. }
{ simpl in H. apply IHT in H. destruct H. exists x.
  split. simpl. right. apply H. apply H. }
{ simpl in H. apply IHT in H. destruct H. exists x.
  split. simpl. right. apply H. apply H. }
 Qed.

Lemma Qty_lt_Qty_ask_m_exists (T: list transaction) (i j:nat):
Qty T i j < Qty_ask T j ->
exists m : transaction,
  In m T /\ idb m <> i /\ ida m = j.
Proof. induction T. simpl. intros. lia.
intros. simpl in H. destruct ((idb a =? i)) eqn:Hi;destruct (ida a =? j) eqn:Hj.
{ simpl in H. assert(Qty T i j < Qty_ask T j). lia. apply IHT in H0. destruct H0. exists x.
  split. simpl. right. apply H0. apply H0. }
{ simpl in H. apply IHT in H. destruct H. exists x.
  split. simpl. right. apply H. apply H. }
{ exists a. split. simpl. auto. move /eqP in Hi. move /eqP in Hj. auto. }
...
Lemma Qty_ask_delete0 (M: list transaction)(t:transaction)(i:nat):
Qty_ask (delete t M) i <=  Qty_ask M i.
Proof. revert t i. induction M as [|m M']. intros t i. simpl. auto.
       intros t i. simpl. 
       destruct (t_eqb t m) eqn: H; destruct (ida m =? i) eqn: H1.  
       lia. lia. simpl. rewrite H1. 
       cut (Qty_ask (delete t M') i <= Qty_ask M' i). lia. apply IHM'.
       simpl. rewrite H1. 
       apply IHM'. Qed.
...
Lemma Qty_bid_delete0 (M: list transaction)(t:transaction)(i:nat):
Qty_bid (delete t M) i <=  Qty_bid M i.
Proof. revert t i. induction M as [|m M']. intros t i. simpl. auto.
       intros t i. simpl. 
       destruct (t_eqb t m) eqn: H; destruct (idb m =? i) eqn: H1.  
       lia. lia. simpl. rewrite H1. 
       cut (Qty_bid (delete t M') i <= Qty_bid M' i). lia. apply IHM'.
       simpl. rewrite H1. 
       apply IHM'. Qed.
...
Lemma Qty_bid_filter M f b: 
Qty_bid (filter f M) (id b) <= Qty_bid M (id b).
Proof.
induction M. simpl. auto.
simpl. destruct (f a) eqn:Ha;destruct (Nat.eqb (idb a) (id b)) eqn:Hb. 
{ simpl. destruct (Nat.eqb (idb a) (id b)). lia. lia. }
{ simpl. destruct (Nat.eqb (idb a) (id b)) eqn:Hab. inversion Hb. auto. }
{ lia. }
{ lia. } Qed.

Lemma Qty_ask_filter M f a: 
Qty_ask (filter f M) (id a) <= Qty_ask M (id a).
Proof.
induction M. simpl. auto.
simpl. destruct (f a0) eqn:Ha;destruct (Nat.eqb (ida a0) (id a)) eqn:Hb. 
{ simpl. destruct (Nat.eqb (ida a0) (id a)). lia. lia. }
{ simpl. destruct (Nat.eqb (ida a0) (id a)) eqn:Hab. inversion Hb. auto. }
{ lia. }
{ lia. } Qed.
...
Lemma Vol_delete_m M m:
In m M -> Vol M = Vol (delete m M) + tquantity m.
Proof. induction M as [|m0 M0]. simpl. firstorder. intros. simpl in H.
destruct H. subst m. simpl. replace (t_eqb m0 m0) with true. lia.
auto. apply IHM0 in H. simpl. destruct (t_eqb m m0) eqn:Hm. 
move /eqP in Hm. subst m. lia. simpl. lia. Qed.
...
Lemma Qty_bid_t_zero_converse (T: list transaction) (i:nat) :
Qty_bid T (i) > 0 -> In (i) (ids_bid_aux T).
Proof. induction T as [|t T']. simpl. lia.
       simpl. intros. destruct (idb t =? i) eqn:Hi. left.
       move /eqP in Hi. auto. right. apply IHT'. auto. Qed.
...
Lemma Qty_ask_t_zero_converse (T: list transaction) (i:nat) :
Qty_ask T (i) > 0 -> In (i) (ids_ask_aux T).
Proof. induction T as [|t T']. simpl. lia.
       simpl. intros. destruct (ida t =? i) eqn:Hi. left.
       move /eqP in Hi. auto. right. apply IHT'. auto. Qed.
...
(* New addition to dsam*)
Lemma Q_by_asks_aux_T1_T2_reverse (T1 T2: list transaction)(I:list nat)
(i:nat) (ndI: NoDup I):
Q_by_asks_aux T1 I = Q_by_asks_aux T2 I -> 
In i I -> (Qty_ask T1 i) > (Qty_ask T2 i) ->
exists i', In i' I/\ (Qty_ask T1 i') < (Qty_ask T2 i').
Proof.  revert T1 T2. induction I as [|j I']. 
        { simpl. intros T1 T2 H0 H1. inversion H1. }
        { intros T1 T2 H1 H2 H3. destruct H2. subst. 
          simpl in H1. assert(Hq: Q_by_asks_aux T2 I' > Q_by_asks_aux T1 I'). 
          lia. apply Q_by_asks_aux_T1_T2 in Hq. destruct Hq as [j Hq].
          exists j. split. simpl. right. apply Hq. destruct Hq. lia. 
          simpl in H1. 
          assert((Qty_ask T1 j < Qty_ask T2 j)\/(Qty_ask T1 j = Qty_ask T2 j)\/
          (Qty_ask T1 j > Qty_ask T2 j)). lia. 
          destruct H0. 
          { exists j. auto. }
          { destruct H0. 
            { assert(HQ: Q_by_asks_aux T1 I' = Q_by_asks_aux T2 I').
              lia. apply IHI' in HQ. destruct HQ as [k HQ].
              destruct HQ. exists k. split. auto. auto. eauto. auto. auto. }
            { assert(HQ: Q_by_asks_aux T1 I' < Q_by_asks_aux T2 I').
              lia. apply Q_by_asks_aux_T1_T2 in HQ. destruct HQ as [k HQ].
              destruct HQ. exists k. split. auto. lia. }
           } } Qed.
...
Proof. revert T1 I1. induction I2. simpl. intros. assert(I1 = nil).
       ...
       assert(I1 [<=] I2). apply Subset_elim3 with (a:=a). auto. auto. eapply IHI2 with (T1:=T1) in H3. simpl. lia.
       eauto. eauto. Qed.
...
(* This section is commented as a part of dsam *)
...
Lemma  Qty_delete (M:list transaction)(m:transaction)(a b : order):
((ida m) <> (id a))\/(idb m) <> (id b) -> 
Qty (delete m M) (id b) (id a) = Qty M (id b) (id a).
Proof. induction M as [| m0 M0]. simpl. auto. simpl. intros.
destruct (t_eqb m m0) eqn:Hm. { move /eqP in Hm. subst m0. 
destruct (idb m =? id b) eqn:Hb; destruct (ida m =? id a) eqn:Ha.
{ destruct H. move /eqP in Ha. destruct H. auto. move /eqP in Hb. destruct H. auto. }
{ simpl. auto. }
{ simpl. auto. }
{simpl. auto. }
}
simpl. destruct (idb m0 =? id b) eqn:Hb; destruct (ida m0 =? id a) eqn:Ha.
{simpl. apply IHM0 in H. lia. }  
{ simpl. apply IHM0 in H. lia. }  
{ simpl. apply IHM0 in H. lia. }  
{simpl. apply IHM0 in H. lia. }  Qed.
